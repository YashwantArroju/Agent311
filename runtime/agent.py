...
SYSTEM_PROMPT = """
    ...
    <guidelines>
        - You MUST call the list_memory_events tool at every turn and see if there is any relevant information that you can use to complete your current task.
    </guidelines>
...
class Agent:
    ...
    async def create_agent(self, actor_id, session_id):
        ...
        # Get the memory ID we'll be working with
    ...
    async def process_query(self, user_query: str, actor_id: str, session_id: str) -> str:
        ...
        # Invoke the agent
        final_state = await agent_executor.ainvoke(initial_input)
        response = final_state['messages'][-1].content
        # response = final_state['messages'][-1].content.replace('\n', '')
        # response = final_state['messages'][-1].content.replace('\n', '\\n')
